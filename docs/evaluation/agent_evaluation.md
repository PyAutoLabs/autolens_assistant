...
| Check 4 — bootstrap from an empty directory (Claude Code, Codex) | n/a | 3 pre-merge (2026-09-26, branch tree URL substituted): Claude Code / claude-opus-5-5 97/100; Codex / gpt-5.6-sol 13/100 twice | Added 2026-09-26 (#138). Claude Code cloned, read `AGENTS.md`, built a Python 3.12 venv, plotted F444W and stopped on the question. Codex made no tool calls: it asked a background question first and the one-turn run ended, so it never reached the bootstrap text. Definitive runs repeat with the real prompt after merge. |
