...
commonly performed with `Lenstool` [@Jullo2007], and other established tools such as `GLEE` and 
`QLens` cover a range of galaxy-, group-, and cluster-scale problems across imaging and 
...
a bespoke analysis. To our knowledge, none currently offer a natural-language or AI-agent 
interface of the kind PyAutoLens-Assistant introduces. PyAutoLens-Assistant is therefore 
complementary to this ecosystem rather than a competitor: the same design — a curated, 
version-controlled knowledge base of skills and reference material layered onto a foundation 
model — could in principle be applied to any of these packages to lower the barrier between a 
scientist's intent and a working analysis.
