...
# ---------------------------------------------------------------------------
def header(title: str, extra: Iterable[str] = ()) -> str:
    lines = [
        f"# {title}",
        "",
        f"Generated by `python autoassistant/chat_bundle.py` on {dt.date.today().isoformat()}.",
...
def build_pack(root: Path, surface: str, names: list[str]) -> dict[str, str]:
    ...
    files["00_instructions.md"] = "\n".join(
        [
            header("PyAutoLens Assistant — chat instructions"),
            "---",
            "",
            rewrite_links(read(root, "AGENTS_CHAT.md"), Path("AGENTS_CHAT.md")),
        ]
    ...
    files["02_skill_index.md"] = "\n".join(
        [
            header("PyAutoLens Assistant — skill index"),
            "",
    ...
    files["03_script_style.md"] = "\n".join(
        [
            header("PyAutoLens Assistant — generated script style"),
            "---",
            "",
    ...
    files["04_wiki_index.md"] = "\n".join(
        [
            header("PyAutoLens reference — wiki index"),
            "---",
            "",
    ...
    api_parts = [header("PyAutoLens reference — API catalogues")]
    ...
    for key, title, group_names in SKILL_GROUPS:
        ...
        if not shipped:
            continue
        parts = [header(f"PyAutoLens skills — {title}")]
        for name in shipped:
